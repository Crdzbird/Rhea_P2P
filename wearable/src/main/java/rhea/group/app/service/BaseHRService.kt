package rhea.group.app.service
...
open class BaseHRService : BaseService(), SensorEventListener {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        heartRateSensor.also { heartRate ->
            sensorManager.registerListener(
                ...
                heartRate,
                272727
            )    // Based on maximum 220bpm, one update per beat
        }
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        ...
        if (newHeartRate == heartRate) return else heartRate =
            newHeartRate    // Only broadcast if HR changes
        broadcastHR()
    }
